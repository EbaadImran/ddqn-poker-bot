...
def command_line_parser():
    ...
    if args['selfplay']:
        ...
        elif args['ebaad_train']:
            runner.train_ebaad_custom(model_name)
        elif args['ebaad_play']:
            runner.play_ebaad_custom(model_name)
...
class SelfPlay:
    ...
    def train_ebaad_custom(self, model_name):
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_random import Player as RandomPlayer
        from agents.agent_ebaad import PokerAgent as EbaadPlayer
        ...
        self.env.add_player(EquityPlayer(name='equity/70/70', min_call_equity=.7, min_bet_equity=.7))
        self.env.add_player(RandomPlayer())
        self.env.add_player(PlayerShell(name='Ebaad', stack_size=self.stack))

        self.env.reset()

        dqn = EbaadPlayer(load_model=model_name, environment=self.env)
        dqn.play(num_episodes=self.num_episodes, render=self.render)

    def play_ebaad_custom(self, model_name):
        """Create 6 players, one of them a trained DQN"""
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_ebaad import PokerAgent as EbaadPlayer
        ...
        dqn = EbaadPlayer(load_model=model_name, environment=self.env, model_path='runs/model_10000_hands.pth')
        dqn.play(num_episodes=self.num_episodes, render=self.render)
